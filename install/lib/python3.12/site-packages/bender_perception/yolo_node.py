#!/home/robotica-06/bender_ws/venv/bin/python
import rclpy
...
venv_python = "/home/robotica-06/bender_ws/venv/bin/python"
if sys.executable != venv_python:
    print(f"🔁 Reiniciando en entorno virtual: {venv_python}")
...
class YoloDepthNode(Node):
    def __init__(self):
        ...
        # Cargar modelo YOLO sólo si la librería está disponible
        self.model = None
        if YOLO is None:
            self.get_logger().warn("⚠️ Paquete 'ultralytics' no disponible. Instala con: pip install ultralytics")
        else:
            try:
                self.model = YOLO("yolov8n.pt")
            except Exception as e:
                self.get_logger().warn(f"⚠️ Error cargando el modelo YOLO: {e}")
                self.model = None
    ...
    def image_cb(self, msg):
        ...
        # Si el modelo YOLO no está disponible, no intentamos detectar
        if self.model is None:
            self.get_logger().debug("YOLO no cargado - saltando detección (instala 'ultralytics' para activarlo)")
            return
